...
from custom_classes import user_data, FuelCalculator, user_language
...
async def fuel_calc(message: Message):
    ...
    if user_language.get(message.from_user.id) == 'RUS':
        keyboard = types.ReplyKeyboardMarkup(keyboard=keyboards.calculator_select, resize_keyboard=True, input_field_placeholder='Выбери тип калькулятора')
    else:
        keyboard = types.ReplyKeyboardMarkup(keyboard=keyboards.calculator_select_en, resize_keyboard=True, input_field_placeholder='Select a type of calculator')
    await message.answer(messages.calculator_selector if user_language.get(message.from_user.id) == 'RUS' else messages.calculator_selector_en, reply_markup=keyboard)
...
async def accure_info_get(message: Message):
    ...
    if user_language.get(message.from_user.id) == 'RUS':
        ...
        await message.answer(messages.accurate_calculator(fuel_flow, lap_time, race_time, answer), parse_mode='Markdown', reply_markup=keyboard)
    else:
        keyboard = types.ReplyKeyboardMarkup(keyboard=keyboards.accure_en, resize_keyboard=True)
        answer = FuelCalculator.accurate_calculation(fuel_flow, lap_time, race_time)
        await message.answer(messages.accurate_calculator_en(fuel_flow, lap_time, race_time, answer), parse_mode='Markdown', reply_markup=keyboard)
...
async def fuel_flow_get(message: Message):
    ...
    await message.answer(messages.get_flow if user_language.get(message.from_user.id) == 'RUS' else messages.get_flow_en,
...
async def lap_time_get(message: Message):
    ...
    if user_language.get(message.from_user.id) == 'RUS':
        ...
        keyboard = types.ReplyKeyboardMarkup(keyboard=keyboards.get_info_calc_en, resize_keyboard=True)
    await message.answer(messages.get_lap_time if user_language.get(message.from_user.id) == 'RUS' else messages.get_lap_time_en,
...
async def race_time_get(message: Message):
    ...
    if user_language.get(message.from_user.id) == 'RUS':
        ...
        keyboard = types.ReplyKeyboardMarkup(keyboard=keyboards.get_info_calc_en, resize_keyboard=True)
    await message.answer(messages.get_race_duration if user_language.get(message.from_user.id) == 'RUS' else messages.get_race_duration_en,
                         parse_mode='Markdown', resize_keyboard=True)
